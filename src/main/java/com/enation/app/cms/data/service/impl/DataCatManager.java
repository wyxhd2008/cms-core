...
import com.enation.app.cms.data.service.ArticleCatRuntimeException;
...
//      判断文章分类不能同名
	...
	public void edit(DataCat cat) {
		...
		//这两个属性并非从页面获取，是空的，如果put进去，sql报错
...
//		map.put("detail_url", cat.getDetail_url());
	...
	public List<DataCat> listAllChildren(Integer parentid) {
		...
		//根据parent_id读取分类数据
		String sql = "select * from es_data_cat where parent_id=?  order by parent_id,cat_order" ;
