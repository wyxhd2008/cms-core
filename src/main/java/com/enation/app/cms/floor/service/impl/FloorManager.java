		...
		for (Panel panel : panelList) {
			...
			for (Map layoutMap : layoutMapList) {
				...
				for(Map block : blockListData){
					...
					}else if(block.get("block_type").equals("MANUAL_GOODS")){
						Double goods_id = (Double) block.get("goods_id");
						int goodsId = goods_id.intValue();
						sql = "select goods_id, goods_name, thumbnail ,price goods_price from es_goods where goods_id = ? and disabled=0 ";
						Map goodsvo = daoSupport.queryForMap(sql, goodsId);
						goodsvo.put("goods_url", "goods-"+goodsId+".html");
						block.put("goods", goodsvo);
					}
				}
