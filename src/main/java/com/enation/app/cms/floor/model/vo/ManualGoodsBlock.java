...
public class ManualGoodsBlock extends Block  implements Serializable  {
	...
	public void convertToElemnet(Element blockEl) {
		if(goods!=null){
			...
			blockEl.select(".goods .g-name").attr("href" ,getContextPath()+"/goods-"+goods.getGoods_id()+".html");
			//对商品价格进行处理，保留两位小数
			double goods_price = goods.getGoods_price();
			DecimalFormat df = new DecimalFormat("#0.00");
			String price = df.format(goods_price);
			blockEl.select(".g-price span").html( StringUtil.toString(price));
		}else{
